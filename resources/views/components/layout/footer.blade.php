@php
    ...
    // Social icons (SVG path per platform). Rendered from the DB-saved social links.
    $socialSvg = [
                    ...
                    <div class="flex gap-4">
                        @foreach ($social as $link)
                            <a href="{{ $link['url'] }}" target="_blank" rel="noopener noreferrer"
                               class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-medical-blue transition-colors"
                               aria-label="{{ ucfirst($link['platform']) }}">
                                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">{!! $socialSvg[$link['platform']] !!}</svg>
                            </a>
                        @endforeach
                    </div>
                @endif
