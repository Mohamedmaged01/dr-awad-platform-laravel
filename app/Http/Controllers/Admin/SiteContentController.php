...
class SiteContentController extends Controller
{
    ...
    private function serviceAttributes(Request $request, array $data, ?Service $existing = null): array
    {
        ...
        return [
            ...
            'features' => $features,
            'icon' => ($data['icon'] ?? null) ?: 'stethoscope',
            'color' => ($data['color'] ?? null) ?: 'from-medical-blue to-cyan-500',
            'price' => $data['price'] ?? null,
            'sort_order' => $data['sort_order'] ?? 0,
            'image_url' => $this->resolveMedia($request, 'image_file', 'image_url', 'services', $existing?->image_url),
            'slug' => $existing?->slug ?? Str::slug(($data['name_en'] ?? null) ?: $data['name_ar']) . '-' . Str::lower(Str::random(4)),
